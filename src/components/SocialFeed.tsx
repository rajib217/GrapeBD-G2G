...
import { Camera, Heart, MessageCircle, Share2, Send, X, Loader2 } from 'lucide-react';
...
const SocialFeed = () => {
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      if (file.size > 10 * 1024 * 1024) { // 10MB limit
        toast({
          title: "ত্রুটি",
          description: "ছবির সাইজ সর্বোচ্চ ১০ এমবি হতে পারে",
          variant: "destructive",
        });
        return;
      }
  ...
  const handleSubmit = async () => {
    if (!newPost.trim() && !selectedImage) {
      toast({
        title: "ত্রুটি",
        description: "কিছু লিখুন বা ছবি যোগ করুন",
        variant: "destructive",
      });
      return;
    }
          ...
          {posts.length === 0 ? (
            <Card>
              <CardContent className="text-center py-8">
                <p className="text-gray-500">এখনো কোন পোস্ট নেই</p>
                <p className="text-sm text-gray-400 mt-1">প্রথম পোস্ট করুন!</p>
              </CardContent>
            </Card>
          ) : (
            posts.map((post) => (
